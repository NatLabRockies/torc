...
/// struct for typed errors of method [`create_user_data_list`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum CreateUserDataListError {
    Status403(models::ErrorResponse),
    Status404(models::ErrorResponse),
...
pub fn create_user_data_list(
    configuration: &configuration::Configuration,
    user_data_list_model: models::UserDataListModel,
) -> Result<models::CreateUserDataListResponse, Error<CreateUserDataListError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_body_user_data_list_model = user_data_list_model;

    let uri_str = format!("{}/bulk_user_data", configuration.base_path);
    let mut req_builder = configuration
        .client
        .request(reqwest::Method::POST, &uri_str);
    ...
    req_builder = configuration.apply_auth(req_builder);
    req_builder = req_builder.json(&p_body_user_data_list_model);
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ...
            ContentType::Text => {
                return Err(Error::from(serde_json::Error::custom(
                    "Received `text/plain` content type response that cannot be converted to `models::CreateUserDataListResponse`",
                )));
            }
            ContentType::Unsupported(unknown_type) => {
                return Err(Error::from(serde_json::Error::custom(format!(
                    "Received `{unknown_type}` content type response that cannot be converted to `models::CreateUserDataListResponse`"
                ))));
            }
        }
    } else {
        let content = resp.text()?;
        let entity: Option<CreateUserDataListError> = serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent {
            status,
            content,
            entity,
