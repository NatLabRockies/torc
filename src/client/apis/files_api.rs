...
pub fn create_files(
    configuration: &configuration::Configuration,
    files_model: models::FilesModel,
) -> Result<models::CreateFilesResponse, Error<CreateFilesError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_body_files_model = files_model;

    let uri_str = format!("{}/bulk_files", configuration.base_path);
    let mut req_builder = configuration
        .client
        .request(reqwest::Method::POST, &uri_str);
    ...
    req_builder = configuration.apply_auth(req_builder);
    req_builder = req_builder.json(&p_body_files_model);
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ...
            ContentType::Text => {
                return Err(Error::from(serde_json::Error::custom(
                    "Received `text/plain` content type response that cannot be converted to `models::CreateFilesResponse`",
                )));
            }
            ContentType::Unsupported(unknown_type) => {
                return Err(Error::from(serde_json::Error::custom(format!(
                    "Received `{unknown_type}` content type response that cannot be converted to `models::CreateFilesResponse`"
                ))));
            }
        }
    } else {
