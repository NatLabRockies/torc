...
impl WorkflowGraph {
    ...
    /// Count independent sub-workflows that may run in parallel.
    pub(crate) fn independent_component_count(&self) -> usize {
        ...
        for start_job in self.nodes.keys() {
            ...
            component_count += 1;
            let mut queue = VecDeque::from([start_job.clone()]);
            while let Some(job) = queue.pop_front() {
                if !visited.insert(job.clone()) {
                    continue;
                }

                if let Some(dependencies) = self.depends_on.get(&job) {
                    queue.extend(
                        dependencies
                            .iter()
...
mod tests {
    ...
    fn test_connected_components_single() {
        ...
        assert_eq!(graph.independent_component_count(), 1);
    }
    ...
    fn test_connected_components_multiple() {
        ...
        let graph = WorkflowGraph::from_spec(&spec).unwrap();
        assert_eq!(graph.independent_component_count(), 2);
    }
