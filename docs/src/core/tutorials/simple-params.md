...
```bash
WORKFLOW_ID=$(torc -f json create parallel_tasks.yaml | jq -r '.workflow_id')
...
```bash
torc -f json jobs list $WORKFLOW_ID | jq -r '.items[].name' | sort
```
...
```bash
WORKFLOW_ID=$(torc -f json create learning_rate_sweep.yaml | jq -r '.workflow_id')
echo "Created workflow: $WORKFLOW_ID"
...
torc -f json jobs list $WORKFLOW_ID | jq '.items | length'
```
