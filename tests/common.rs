...
pub fn create_diamond_workflow(
    ...
    if init_jobs {
        default_api::initialize_jobs(config, workflow_id as i64, None, None)
...
pub fn create_minimal_resources_workflow(
    ...
    if init_jobs {
        default_api::initialize_jobs(config, workflow_id, None, None)
            .expect("Failed to initialize jobs");
    }
...
pub fn delete_all_workflows(config: &Configuration) -> Result<(), Box<dyn std::error::Error>> {
    ...
    for workflow in workflows {
        if let Some(workflow_id) = workflow.id
            && let Err(e) = default_api::delete_workflow(config, workflow_id)
        {
