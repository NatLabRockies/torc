...
or define a custom profile under `client.hpc.custom_profiles` (see the `torc-config` skill), then
pass `--profile <name>` explicitly.
...
| `--group-by partition` (default)   | Jobs mapping to the same partition share one scheduler, fewer allocations      |
...
The trade-off is queue priority against allocation reuse:

- Short walltime (`max-job-runtime`) usually gets scheduled sooner, but each allocation drains fewer
  sequential jobs, so you need more allocations.
- Long walltime (`max-partition-time`) packs more jobs per allocation but may sit in the queue.
...
past workflow completion with `compute_node_ignore_workflow_completion`. `serialize_allocations` on
a scheduler chains its allocations one at a time, which requires a single fixed Slurm job name (so
`--job-prefix` is rejected for that scheduler).

## Node packing

A ready job will not start on a running allocation whose remaining walltime is shorter than the
job's `runtime`, because it would be killed mid-run. Packing therefore degrades silently as
allocations age.
